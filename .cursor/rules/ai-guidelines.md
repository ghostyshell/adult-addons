...
| User-facing copy, de-AI text, naturalize UI/marketing strings | **humanizer** + **no-em-dashes** (`.cursor/rules/no-em-dashes.mdc`) - no em/en dashes in public text |
...
**Typical flow:** ui-ux-pro-max, design-taste-frontend, or bencium-* for direction → implement (motion-* or react-view-transitions when animating) → **humanizer** + **no-em-dashes** on user-facing copy → web-design-guidelines + accesslint-* before shipping.
